...
describe('ops dashboard + alerts reference real log output', () => {
  ...
  it('every field they filter, unwrap or group by is written by the logger or the service', () => {
    ...
    for (const f of ['to', 'from', 'lane', 'msInPreviousStatus', 'payToDeliveredMs', 'memberReview', 'memberReviewOldestAgeSeconds', 'rescueAvailable', 'parentKnown', 'approved', 'square', 'advisoryFails', 'retryable', 'via'])
    ...
    const written = (f: string) => new RegExp(`[{,\\s]${f}\\s*[:,}]`).test(code) || new RegExp(`\\b${f}\\b`).test(SOURCES[0]!);
    expect([...fields].filter((f) => !written(f))).toEqual([]);
  });
