...
| `mint gauges` | `MintWorker.run`, at most once a minute (the only periodic gauge line) | `memberReview`, `memberReviewOldestAgeSeconds`, `rescueAvailable`, `queued`, `revealing`, `awaitingConfirmation`, `parentKnown`, `parentConfirmed`, `parentLeased` (0/1); `counts.<status>` and `oldestSeconds.<status>` for every non-terminal status, which `\| json` flattens to `counts_<status>` / `oldestSeconds_<status>` |
...
| Oldest order per status | gauges `oldestSeconds_<status>` (`paid` … `verified`) | How long the oldest order has sat in each status right now; a flat line that keeps rising is a stuck order. |
...
| `DegentMintOrderStuck` | gauge `oldestSeconds_revealed` > 3 h (reveal not confirming) | warning | RUNBOOK §1 |
