...
describe('worker: mint gauges per status (alerting feed, docs/DEPLOY.md + products/degent/ops)', () => {
  it('counts every non-terminal status and the age of the oldest order in each', async () => {
    const h = makeHarness();
    await h.ready;
    let g = await h.worker.gauges();
    expect(g.parentKnown).toBe(true);
    expect(g.counts.member_review).toBe(0);
    expect(g.counts).not.toHaveProperty('delivered');
    ...
    await fundToReview(h, b);
    h.clock.advance(30);
    g = await h.worker.gauges();
    expect(g.counts.awaiting_payment).toBe(2);
    expect(g.oldestSeconds.awaiting_payment).toBe(150);
    expect(g.counts.member_review).toBe(1);
    expect(g.oldestSeconds.member_review).toBe(30);
    expect(g.memberReview).toBe(1);
    expect(a.orderId).toBeTruthy();
  });

  it('run() logs one "mint gauges" line carrying the per-status counts', async () => {
    const lines: Array<{ msg: string; fields?: Record<string, unknown> }> = [];
    const h = makeHarness();
    await h.ready;
    const { MintWorker, LOG_GAUGES } = await import('../src/worker.js');
    ...
    stop.abort();
    await done;
    const gauges = lines.filter((l) => l.msg === LOG_GAUGES);
    expect(gauges).toHaveLength(1);
    expect(gauges[0]?.fields).toMatchObject({ parentKnown: 1, counts: { queued: 0 }, oldestSeconds: { queued: 0 } });
    expect(lines.some((l) => l.msg === 'order status snapshot')).toBe(false);
  });
});
