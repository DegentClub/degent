...
- Questions and ideas: [Discussions](https://github.com/DegentClub/degent/discussions) (see [SUPPORT.md](SUPPORT.md)).
- Bugs, features and wallet compatibility reports: [issue forms](https://github.com/DegentClub/degent/issues/new/choose).
...
```bash
git clone --recurse-submodules https://github.com/DegentClub/degent.git
corepack enable
pnpm install
pnpm check        # validate manifests + boundaries + typecheck + tests: exactly what CI runs
```

The shared platform (`@bsh/*` libraries, the catalog tool, the component templates) comes from
[DegentClub/scribbit](https://github.com/DegentClub/scribbit), pinned as a git submodule at `deps/scribbit`.
**Never edit `deps/scribbit` here.** Fix the platform in DegentClub/scribbit first; once merged, bump the pin in a
separate PR (`git -C deps/scribbit fetch && git -C deps/scribbit checkout <commit> && pnpm install && pnpm check`).
...
Copy a skeleton from `deps/scribbit/templates/` (`library`, `service` or `app`) to `products/degent/{packages,services,apps}/<name>/`:

```bash
cp -r deps/scribbit/templates/library products/<product>/packages/<name>
...
The code here is Apache-2.0; the Degent artwork, the comic, the collection's images, the degent.club names and
logo and the gold frame and placard design are **not** (see [ART-AND-TRADEMARKS.md](ART-AND-TRADEMARKS.md)). Do not
add artwork, collection images or brand assets in a pull request, and do not submit third-party art. Test fixtures
